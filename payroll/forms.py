...
from .models import (Bank , Company , Deduction , Department , EmployeeDeduction , PaymentMethod , Salary , Employee ,
                     Allowance ,
...
class EmployeeDeductionCreateForm( forms.ModelForm ):
	ded_date = forms.DateField( required=True , widget=DateInput() , label="Deduction Date" )
	...
	def __init__ (self , *args , **kwargs):
		...
		self.helper.add_input(
			Submit( 'submit' , 'Create Deduction' , css_class='btn btn-outline-info' ) )

	class Meta:
		model = EmployeeDeduction
		fields = ['employee' , 'deduction' , 'amount' , 'ded_date' , 'recurring' , 'comment' , ]
...
class SalaryCreateForm( forms.ModelForm ):
	# pay_period_start = forms.DateField( required=False , widget=DateInput() )
	pay_period_end = forms.DateField( required=False , widget=DateInput() )
...
TimesheetDetailFormset = inlineformset_factory(
	...
	fields=['employee' , 'date_time_in' , 'date_time_out' , 'hours'] , extra=0 , can_delete=True , validate_min=True ,
	min_num=1 )
